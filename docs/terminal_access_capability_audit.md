...
| **Quotes/ticks (live)** | `symbol_info_tick` (poll legado); WS feed via MQL5 Service (adaptador) | **WS feed:** sim, quando `feed.enabled=True` (Service → gateway → QuoteTick). **Poll:** sim, só com `feed.enabled=False` | ✅ TC-D20 wiring; ✅ feed Tier 1 (`test_feed_*`); ✅ TC-HOM-D02 live stream | **Partial** — live stream real validado via WS; poll legado mantido para transição |
