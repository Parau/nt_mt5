...
| Quote ticks | Partial | TC-D20 | **Live WS feed:** Tier 1 matrix + homolog. **Legacy poll:** TC-D20 wiring. | ✅ **Tickmill:** TC-HOM-D02 (`run_open_market.py`, BTCUSD, 2026-06-28). **XP/B3:** TC-HOM-D02 (`run_xp_open_market_feed.py`, WDON26, 201 ticks/60s, 2026-06-29); TC-HOM-D07 multi-symbol 4/4 (`run_xp_open_market_feed.py` 2026-06-29; `run_wave3_homologation.py` PETR4=99, 2026-06-30) | `homologation/run_open_market.py`, `run_xp_open_market_feed.py`, `run_wave3_homologation.py`, `run_feed_smoke.py` | Live path via MQL5 Service WS when `feed.enabled=True`. D07 requer pregão aberto por classe de ativo (ex.: PETR4 equity vs futuros B3). |
...
| Trade ticks | **Profile-dependent** | TC-D30 | **Tickmill:** rejeição via `VenueProfile` (`test_tc_d30_*`). **XP_B3:** subscribe passa gate + homolog open (`homologation/scenarios/trade_tick_open_market.py`). | **Tickmill:** N/A (`last=0`). **XP/B3:** TC-HOM-D30 WINQ26 (`run_xp_trade_ticks_homologation.py`, 2026-06-30). Nominais only — `WIN$`/`WDO$` excluded (continuous, no bid/ask) | `test_data_tester_matrix_external_rpyc.py::test_tc_d30_*`, `homologation/run_xp_trade_ticks_homologation.py` | **Tickmill = Unsupported** (by design). **XP_B3:** live `SubscribeTradeTicks` homologado WINQ26 em pregão aberto. |
| Historical trade ticks | **Profile-dependent** | TC-D31 | **Tickmill:** FOREX/CFD rejeitados (`test_tc_d31_*`). **XP_B3:** `WINQ26`/`WDON26` entrega `TradeTick` via fake bridge + homolog | **Tickmill:** N/A. **XP/B3:** TC-HOM-D31 open WINQ26 (`run_xp_trade_ticks_homologation.py`, 2026-06-30); closed WINQ26/WDON26 via `run_xp_closed_market.py` | `test_data_tester_matrix_external_rpyc.py::test_tc_d31_*`, `homologation/run_xp_trade_ticks_homologation.py`, `homologation/run_xp_closed_market.py` | **Tickmill = Unsupported.** **XP_B3:** histórico open + closed homologados (nominais). |
| Bars | Partial | TC-D40, TC-D41, TC-HOM-D03, TC-HOM-D04b | TC-D40: hist `_request_bars` via `copy_rates_*` (wiring + stub e2e + fake-bridge e2e); TC-D41: live subscribe via WS `subscribe_bars` when `feed.enabled` (Tier 1 matrix); TC-HOM-D03 live smoke `homologation/run_bar_smoke.py`; TC-HOM-D04b closed-market `_request_bars` E2E | ✅ **Tickmill:** live M1 bar via WS (BTCUSD, 2026-06-28). **XP/B3:** TC-HOM-D03 (`run_xp_open_market_feed.py`, WDON26 M1, 2026-06-29) | `test_data_tester_matrix_external_rpyc.py`, `test_feed_data_client.py`, `homologation/run_bar_smoke.py`, `homologation/run_xp_open_market_feed.py` | Live bar subscribe **requires** `feed.enabled=True` + `NT5TickFeedService`. |
