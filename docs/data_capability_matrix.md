...
| Quote ticks | Partial | TC-D20 | **Live WS feed:** `test_feed_data_client.py`, `test_feed_gateway.py`, `test_mt5_client_live_feed_poll.py` (gateway subscribe, no RPyC BidAsk poll when `feed.enabled=True`). **Legacy poll:** TC-D20 wiring em `test_data_tester_matrix_external_rpyc.py`. | ✅ **Live stream** TC-HOM-D02 via `homologation/run_feed_smoke.py` (MQL5 Service → WS → QuoteTick, Tickmill-Demo BTCUSD, 2026-06-27). Legacy poll USTEC/bid-ask em `test_external_rpyc_data_tester.py`. | `test_feed_data_client.py`, `test_feed_gateway.py`, `homologation/run_feed_smoke.py` | **Live path (2026-06-27):** com `feed.enabled=True`, quotes live vêm do MQL5 Service (`CopyTicks` + cursor) via WebSocket; RPyC poll `symbol_info_tick` desactivado (Fase 4). **Legacy:** `feed.enabled=False` mantém poll ~1 Hz (snapshot, não streaming fiel). Primeiro tick após subscribe pode ter bid/ask zero no path legado; ignorar antes de operar. TC-HOM-D02 exige `MT5_FEED_ENABLED=1` + Service MT5.
...
| Lifecycle / unsubscribe / custom params | Partial | TC-D70–TC-D72 | TC-D70: `_unsubscribe_quote_ticks` → WS when feed enabled; `_unsubscribe_bars` → WS `unsubscribe_bars` when feed enabled (Tier 1 matrix). TC-D71/D72 documented as unsupported. | Pendente | `test_data_tester_matrix_external_rpyc.py::test_tc_d70_*` | Quote unsubscribe on stop validated in TC-HOM-D05. Bar unsubscribe on stop validated when feed enabled (same harness). |
