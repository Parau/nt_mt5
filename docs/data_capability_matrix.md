...
| Historical trade ticks | **Profile-dependent** | TC-D31 | **Tickmill:** FOREX/CFD rejeitados (`test_tc_d31_*`). **XP_B3** + **AMP_US:** nominais entrega `TradeTick` via fake bridge + homolog | **Tickmill:** N/A. **XP/B3:** WINQ26, **PETR4**, **DI1F27** open (`run_xp_symbol_quote_trade_confirm.py`, 2026-06-30). **AMP:** MESU26 etc. (`run_amp_symbol_quote_trade_confirm.py`) | `homologation/run_xp_trade_ticks_homologation.py`, `homologation/run_amp_trade_ticks_homologation.py` | Histórico open homologado para nominais XP e CME AMP. |
