...
| Quote ticks | Partial | TC-D20 | **Live WS feed:** Tier 1 matrix + homolog. **Legacy poll:** TC-D20 wiring. | ✅ TC-HOM-D02 via `run_open_market.py` (Tickmill BTCUSD, 2026-06-28, 126 ticks/60s WS) | `homologation/run_open_market.py`, `run_feed_smoke.py` | Live path via MQL5 Service WS when `feed.enabled=True`. |
| Historical quote ticks | Partial | TC-D21 | TC-D21 coberto em `test_data_tester_matrix_external_rpyc.py`: wiring + end-to-end + `limit=0` usa `tick_capacity`; **`start=None` com `limit` explícito honra o limit** + resultado vazio não chama `_handle_quote_ticks` + `correlation_id` repassado. Live Nautilus-level: `homologation/run_open_market.py` (TC-HOM-D21). | ✅ TC-HOM-D21 via `run_open_market.py` / `run_closed_market.py` (Tickmill BTCUSD) | `test_data_tester_matrix_external_rpyc.py::test_tc_d21_*`, `homologation/run_open_market.py` | **Fix D21 (2026-06-28):** path canônico `get_historical_ticks` → **`copy_ticks_from`** (IB `req_historical_ticks` removido). `start=None` + `limit>0` respeita o limit; `tick_capacity` só quando `limit=0`. RPyC: preferir timestamps Unix `int`. |
| Trade ticks | **Profile-dependent** | TC-D30 | **Tickmill:** rejeição via `VenueProfile` (`test_tc_d30_*`). **XP_B3:** subscribe passa gate (`test_tc_d30_xp_subscribe_trade_ticks_reaches_client`). | Tickmill: confirmado `last=0.0` CFD/FX. XP: histórico OK (closed); live WS **OPEN** (pregão B3). | `test_data_tester_matrix_external_rpyc.py::test_tc_d30_*` | **Tickmill = Unsupported** (by design). **XP_B3 = Partial** (OBSERVED): histórico + subscribe wiring; live stream pendente pregão aberto. |
| Historical trade ticks | **Profile-dependent** | TC-D31 | **Tickmill:** FOREX/CFD rejeitados (`test_tc_d31_*`). **XP_B3:** `WIN$` entrega `TradeTick` via fake bridge (`test_tc_d31_xp_request_trade_ticks_delivers_trade_tick_objects`). | XP: TC-HOM-D21-T (closed market). Tickmill: N/A | `test_data_tester_matrix_external_rpyc.py::test_tc_d31_*` | **Tickmill = Unsupported.** **XP_B3 = Partial** (OBSERVED): histórico homologado pregão fechado; live pendente. |
| Bars | Partial | TC-D40, TC-D41, TC-HOM-D03, TC-HOM-D04b | TC-D40: hist `_request_bars` via `copy_rates_*` (wiring + stub e2e + fake-bridge e2e); TC-D41: live subscribe via WS `subscribe_bars` when `feed.enabled` (Tier 1 matrix); TC-HOM-D03 live smoke `homologation/run_bar_smoke.py`; TC-HOM-D04b closed-market `_request_bars` E2E | ✅ Live M1 bar via WS feed (BTCUSD, Tickmill-Demo, 2026-06-28); hist M1/M5 via `copy_rates_from_pos` (USTEC) | `test_data_tester_matrix_external_rpyc.py`, `test_feed_data_client.py`, `homologation/run_bar_smoke.py`, `homologation/scenarios/closed_market_suite.py` | Live bar subscribe **requires** `feed.enabled=True` + `NT5TickFeedService`. IB `req_real_time_bars` / `cancel_historical_data` removed from subscribe and on-demand hist paths. Wire timeframes: M1, M5, M15, M30, H1, H4, D1. Sub-second (5s) bars unsupported on WS. On-demand `_request_bars` uses `copy_rates_from_pos` / `copy_rates_range` (D04b). |
...
| Lifecycle / unsubscribe / custom params | Partial | TC-D70–TC-D72 | TC-D70 Tier 1 matrix. TC-D71/D72 documented unsupported. | ✅ TC-HOM-D05 via `run_open_market.py` (2026-06-28) | `homologation/run_open_market.py` | Quote + bar unsubscribe on stop validated open-market. |
