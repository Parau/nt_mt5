...
| Historical trade ticks | **Profile-dependent** | TC-D31 | **Tickmill:** FOREX/CFD rejeitados (`test_tc_d31_*`). **XP_B3** + **AMP_US:** nominais entrega `TradeTick` via fake bridge + homolog. **A05 bounded:** `start/end + limit=0` → `copy_ticks_range(COPY_TICKS_TRADE)` + live routing; empty ndarray → `DataResponse([])`; `None` → `MT5HistoricalDataError` (`test_a05_*`, `test_a05_bounded_*`). **Live AMP ENQU26 (2026-08-10):** A05 bounded PASS (9212 ticks / empty success); bridge precisa `allow_pickle` para materializar ndarray local | **Tickmill:** N/A. **XP/B3:** WINQ26, **PETR4**, **DI1F27** open. **AMP:** MESU26 etc. + **ENQU26 A05/D30/D31 (2026-08-10)**. **A05 live↔hist parity stream:** harness ready, full parity gate still optional follow-up | `test_data_tester_matrix_external_rpyc.py::test_tc_d31_*`, `test_a05_bounded_trade_ticks.py`, `homologation/run_amp_a05_bounded.py`, `homologation/run_amp_trade_ticks_homologation.py` | Histórico count-based + A05 bounded live AMP PASS. EXTERNAL_RPYC exige bridge com `allow_pickle` (patch em `/opt/bridge` + vendor). |
