---
ID: 019fed0b-6db0-722d-8fbb-81d7449bc4ed
---

# A05 implementation clarifications for the `nt_mt5` coding agent
...
This document **clarifies implementation details only**. It does not reopen A05
or B07 architecture. A05 remains the adapter specification.
...
# 14. Canonical bounds and provider envelope
...
flags = getattr(mt5, "COPY_TICKS_TRADE", None)

if flags is None:
    raise MT5HistoricalDataError(
        "MT5 terminal surface does not expose COPY_TICKS_TRADE",
    )
...
Do not silently substitute `COPY_TICKS_ALL`.
