"""
TC-HOM-E10 / E10b: multi-leg position reconciliation and close-on-stop.

E10 opens a known hedging book (2× BUY + 1× SELL), compares bridge
``positions_get`` with ``generate_mass_status`` position reports.

E10b verifies ``close_on_stop`` clears multiple open legs on disconnect.
"""
...
async def _market_ioc(
    ...
    order = MarketOrder(
        ...
        strategy_id=StrategyId("HOMOLOG-E10"),
