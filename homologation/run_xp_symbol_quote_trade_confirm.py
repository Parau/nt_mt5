"""Confirm QuoteTick + TradeTick (live D02/D30 + hist D21/D31) per XP symbol."""
...
def _symbols() -> tuple[str, ...]:
    raw = os.environ.get("HOMOLOG_CONFIRM_SYMBOLS", "PETR4,DI1F27").strip()
...
async def main() -> int:
    ...
    print("=" * 64)
    print("  XP/B3 — Quote + Trade tick confirm (D02/D21/D30/D31)")
    print(f"  Symbols : {','.join(symbols)}")
    print(f"  Gateway : {os.environ.get('MT5_HOST', '127.0.0.1')}:{os.environ.get('MT5_PORT', '18813')}")
    print("=" * 64)
    ...
    path = os.environ.get(
        "HOMOLOG_REPORT_JSON",
        "homologation/last_xp_petr4_di1f27_data_confirm.json",
    )
