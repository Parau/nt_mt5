"""XP/B3 open-market TradeTick homologation — TC-HOM-D30 + TC-HOM-D31."""
...
async def main() -> int:
    ...
    print("=" * 64)
    print("  MT5 ADAPTER — XP/B3 TRADE TICK HOMOLOGATION (D30/D31)")
    print(f"  Gateway : {cfg.host}:{cfg.port}")
    print(f"  Profile : {cfg.venue_profile.name}")
    print(f"  Symbols : {os.environ.get('HOMOLOG_TRADE_SYMBOLS', 'WINQ26')}")
    print("=" * 64)
    ...
    path = os.environ.get(
        "HOMOLOG_REPORT_JSON",
        "homologation/last_xp_trade_ticks_report.json",
    )
